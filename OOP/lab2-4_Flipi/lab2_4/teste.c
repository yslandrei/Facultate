...
void teste_domain()
{
	...
	//teste copy
	Masina copy_masina;
	copy_masina = copiaza_masina(masina);
	...
	assert(strcmp(masina.categorie, masina_noua.categorie) == 0);
	assert(strcmp(masina.model, masina_noua.model) == 0);
...
void teste_service()
{
	...
	assert(modificat == 1);
	assert(strcmp(lista.elemente[0].model, "passat") == 0);
	...
	assert(lista.elemente[0].inchiriere == 1);
	assert(succes1 == 1);
	...
	assert(lista.elemente[0].inchiriere == 0);
	assert(succes2 == 1);
	...
	filtered_list = serv_filtrare(&lista, "sport");
	assert(size(&filtered_list) == 0);
	filtered_list = serv_filtrare(&lista, "suv");
