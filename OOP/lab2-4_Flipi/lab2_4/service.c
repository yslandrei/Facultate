...
int cauta_id(Lista* lista, char* nr_inmatr)
{
	...
	for (int i = 0; i < lista->lungime; i++)
	{
		...
		if (strcmp(lista->elemente[i].nr_inmatriculare, nr_inmatr) == 0)
		{
			pozitie = i;
			break;
		}
...
int serv_inchiriaza(Lista* lista, char* nr_inmatr)
{
	...
	if (pozitie != -1)
	{
		lista->elemente[pozitie].inchiriere = 1;
		return 1;}
	else return 0;
}
...
int serv_returnare(Lista* lista, char* nr_inmatr)
{
	...
	if (pozitie != -1)
	{
		lista->elemente[pozitie].inchiriere = 0;
		return 1;}
	else return 0;
}
...
Lista serv_filtrare(Lista* lista, char* categ)
{
	...
	if (strcmp(categ, "sedan") == 0 || strcmp(categ, "suv") == 0 || strcmp(categ, "sport") == 0 || strcmp(categ, "mini") == 0)
	{
		for (int i = 0; i < lista->lungime; i++)
		{
			if (strcmp(lista->elemente[i].categorie, categ) == 0)
			{
