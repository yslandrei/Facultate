...
class ClosedGui : public observer {
private:
...
public:
	ClosedGui(taskService& tService) : tService{ tService } {
		initGui();
		initConnections();
		loadList(tService.getAllClosed());
	}
	...
	void initConnections() {
		...
		QObject::connect(openBtn, &QPushButton::clicked, [&]() {
			try {
				vector<task> tList = tService.getAllClosed();
