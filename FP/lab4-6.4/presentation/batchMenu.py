...
def parseCommands(com):
    comList = []
    comList = com.split('; ')
    return comList, len(comList)
...
def batchMenu():
    ...
    while(1):
        printMenu()
        com = input("Comanda (a = afis, u = undo, q = quit, ; = delimitator): ")
        comList, comListLength = parseCommands(com)
        print("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n")
        for i in range(0, comListLength):
            com = comList[i]
            if com == "q":
                return
            elif com[:4] == "add ":
